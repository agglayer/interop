...
macro_rules! match_res {
    (
        ...
        $crate::match_res!($expr
            $(, Ok($ok) => $ok_handler)*
            $(, Err(type<$err_ty>($err)) => $err_handler)*
            , Err(else(err)) => { return Err(err) }
        )
    };
    ...
    (
        ...
        match $expr {
            ...
            Err(err) => $crate::match_err!(err,
                $(
                    type<$err_ty>($err) => $err_handler,
                )*
                else($default_err) => $default_handler,
            ),
        }
    };
}
...
mod tests {
    ...
    fn use_match_res_without_default(res: eyre::Result<u32>) -> eyre::Result<String> {
        match_res!(res,
            ...
            Ok(n) => Ok(format!("ok({n})")),
            Err(type<Bar>(Bar("foo"))) => Ok("bar/foo".to_string()),
            Err(type<Bar>(b)) => Ok(format!("bar({})", b.0)),
